...
set QUARTUS_INSTALL_DIR [lindex $argv 2]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {intel_rtile_cxl_top_cxltyp3_ed::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [intel_rtile_cxl_top_cxltyp3_ed::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]

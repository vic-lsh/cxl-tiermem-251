...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [emif2::get_common_design_files "$QSYS_SIMDIR"]]
...
set QUARTUS_INSTALL_DIR [lindex $argv 2]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {emif2::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [emif2::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
